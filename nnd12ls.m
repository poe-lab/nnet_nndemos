...
%  initial values, and watch as the network is trained with conjugate
%  gradient line search to find a minimum on the error surface. 
%
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  % NEW DEMO FIGURE
  fig = nndemof2(me,'DESIGN','Conj. Gradient Line Search','','Chapter 12');
  set(fig, ...
  ...
  H = nndArray({fig_axis desc_text cont_axis cont_ptr ...
...
elseif strcmp(cmd,'down') & ~isempty(fig) & (nargin == 1)
  ...
  if (x > xlim(1) & x < xlim(2) & y > ylim(1) & y < ylim(2))
    ...
    t=(.01:.01:1)'*2*pi;
    circ_x1=sin(t)*.1*circ_gain;
    circ_y1=cos(t)*.1*circ_gain;
    circ_x2=sin(t)*.2*circ_gain;
    circ_y2=cos(t)*.2*circ_gain;
    circ_x3=sin(t)*.3*circ_gain;
    circ_y3=cos(t)*.3*circ_gain;
    circ_x4=sin(t)*.4*circ_gain;
    circ_y4=cos(t)*.4*circ_gain;
    ...
    for epoch=1:(max_epoch)
      ...
      fb = sumsqr(T - logsig(W2t*logsig(W1t*P+b1t*ones(1,Q))+b2t*ones(1,Q)));
        xc=circ_x2+newx;
        yc=circ_y2+newy;
        path = [path; fill(xc,yc,'w','facecolor','none','CreateFcn','','EdgeColor','b')];
      end
      xc=circ_x2+newx;
      yc=circ_y2+newy;
      path = [path; fill(xc,yc,'w','facecolor','none','CreateFcn','','EdgeColor','b')];
      ...
      xc=circ_x3+newx;
      yc=circ_y3+newy;
      path = [path; fill(xc,yc,'k','facecolor','none','CreateFcn','','EdgeColor','k')];
      ...
      xc=circ_x1+newx;
      yc=circ_y1+newy;
      path = [path; fill(xc,yc,'k','CreateFcn','','EdgeColor','k')];
      d=b-tau1*(b-a);
      if (option == 1)
      ...
      xc=circ_x1+newx;
      yc=circ_y1+newy;
      path = [path; plot(xc,yc,'.','color',nnred,'CreateFcn','','markersize',15)];
    ...
    xc=circ_x2+optx;
    yc=circ_y2+opty;
    path = [path; fill(xc,yc,'w','CreateFcn','','EdgeColor','b')];
