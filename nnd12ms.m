...
%  initial values, and watch as the network is takes one step with
%  Marquardt-Levenberg backpropagation. 
%
...
elseif strcmp(cmd,'down') & ~isempty(fig) & (nargin == 1)
  ...
  if (x > xlim(1) & x < xlim(2) & y > ylim(1) & y < ylim(2))
    ...
    xc=circ_x2+optx;
    yc=circ_y2+opty;
    % path = [path; fill(xc,yc,'w','CreateFcn','','EdgeColor',nnred)];
    drawnow
    ...
    for k=1:(max_epoch)
      ...
      while tst>0,
        ...
        xc=circ_x2+newx;
        yc=circ_y2+newy;
        path = [path; plot([Lx1 newx],[Ly1 newy],'-b','linewidth',1,'CreateFcn','')];
        Lx1 = newx;
        Ly1 = newy;
