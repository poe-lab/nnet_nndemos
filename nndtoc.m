...
% Copyright 1994-2011 Martin T. Hagan and Howard B. Demuth. Used by Permission.
...
if strcmp(cmd,'init') & ~isempty(fig)
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  % SAVE DATA
  H = nndArray({fig_axis icon_ptr text_ptr menu_ptr});
  set(fig,'userdata',H);
...
elseif strcmp(cmd,'menus') & (nargin == 2) & ~isempty(fig)
...
elseif strcmp(cmd,'select') & (nargin == 2) & ~isempty(fig)
