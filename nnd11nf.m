...
%
%  Alter the input-output function of a two-layer feedforward network
%  by choosing weight and bias values and the output transfer function.
%
%  NND11NF
...
if ~isempty(fig)
  ...
  indicators = H(10:16);      % parameter indicators
...
elseif strcmp(cmd,'init') & isempty(fig)
    ...
    for j=1:4
      ...
      if (i ~= 1) | (j ~= 4)
        ...
        set(ax,...
          ...
          'xticklabel',char(['-' maxstr],'','0','',maxstr));
  ...
  H = nndArray({fig_axis desc_text meters indicators  tf_icon_ptr tf_menu ...
...
elseif strcmp(cmd,'tf') & ~isempty(fig)
...
elseif strcmp(cmd,'f_motion') & ~isempty(fig)
