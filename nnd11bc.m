...
%
%  Choose an input, randomize the weights and watch the calculations
%  as a two-layer feedforward network calculates its output, error
%  backpropagates its derivatives of error, and adjusts its weights
%  and bias.
%
...
%%%%%%%%
  ...
  y = 181;
  dy = 17;
  init_str = char('p','t','a1','a2','e','s2','s1','W1','b1','W2','b2');
  vars = zeros(1,11);
  vals = vars;
  for i=1:11
    yy = y-dy*(i-1);
  ...
  'position',[156 173 60 18],...
    'style','edit',...
  'string','1.0',...
    'ToolTipStr','You can enter the input into this box.',...  %mth 1/25/05
    'callback',[me '(''p'')']);
  ...
  go_button = uicontrol(...
    ...
    'string','Target',...
    'ToolTipStr','Push this button to continue the demonstration.',...  %mth 1/25/05
    'callback',[me '(''go'')']);
  uicontrol(...
    'units','points',...
    'position',[410 130 60 20],...
    'string','Reset',...
    'ToolTipStr','This button resets the demonstration.',...  %mth 1/25/05
    'callback',[me '(''reset'')']);
  uicontrol(...
    'units','points',...
    'position',[410 105 60 20],...
    'string','Random',...
    'ToolTipStr','This button randomizes the weights.',...  %mth 1/25/05
    'callback',[me '(''random'')'])
  uicontrol(...
    'units','points',...
    'position',[410 80 60 20],...
    'string','Contents',...
    'ToolTipStr','This button takes you back to the Table of Contents window.',...  %mth 1/25/05
    'callback','nndtoc')
  uicontrol(...
    'units','points',...
    'position',[410 55 60 20],...
    'string','Close',...
    'ToolTipStr','This button closes the current window.',...  %mth 1/25/05
    'callback',[me '(''close'')'])
...
elseif strcmp(cmd,'p') & ~isempty(fig)
...
elseif strcmp(cmd,'go') & ~isempty(fig)
