...
%
%  Choose the number of neurons and difficulty of a function to fit,
%  then watch as the network is trained to fit points on the function
%  and generalize between the points.
%
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  % NEW DEMO FIGURE
  fig = nndemof2(me,'DESIGN','Generalization','','Chapter 11');
  set(fig, ...
  ...
  H = nndArray({fig_axis desc_text s1_bar s1_text fa_axis fa_ptr i_bar i_text});
...
elseif strcmp(cmd,'train') & ~isempty(fig) & (nargin == 1)
  ...
  pmax = max(P')';
  magw = 2.8*S1^(1/R);
  W10 = magw*randnr(S1,R);
  B10 = magw*rands(S1,1);
  rng = pmax-pmin;
  mid = 0.5*(pmin+pmax);
  W10 = 2*W10./(ones(S1,1)*rng');
  B10 = B10 - W10*mid;
  ...
  axes(fa_axis)
  set(get(fa_axis,'children'),'CreateFcn','');
  delete(get(fa_axis,'children'))
  A = W20*logsig(W10*P2+B10*ones(1,Q2))+B20*ones(1,Q2);
  Target = plot(P,T,'+','color',nnred,'CreateFcn','');
  ...
  Attempt = plot(P2,AA,'-','color',nndkblue,'linewidth',2,'CreateFcn','');
  drawnow
