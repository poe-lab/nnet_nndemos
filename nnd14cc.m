...
%
%  See a competitive layer classify oranges and apples based on
%  their similarity to two prototype vectors.
%
...
if ~isempty(fig)
  ...
  angle_ptr = H(21);       % angle (deg) of lines in conveyor ends
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  % NEW DEMO FIGURE
  fig = nndemof2(me,'DESIGN','Competitive Classification','','Chapter 14');
...
elseif strcmp(cmd,'fruit') & ~isempty(fig) & (nargin == 1)
  ...
  t1 = clock;
  nnsound(wind,Fs);
  while nndpause(clock,t1) < 0.5, end
  nndpause(0.25)
  set(arrows(1),...
    'facecolor',nndkblue)
  ...
  for dx=10:10:60
    t1 = clock;
    nnsound(blp,Fs);
    while nndpause(clock,t1) < 0.05, end
    set(fruit,...
      'facecolor',nnltgray,...
      'edgecolor',nnltgray);
    set(fruit,...
...
elseif strcmp(cmd,'remove') & ~isempty(fig) & (nargin == 1)
...
elseif strcmp(cmd,'setup') & ~isempty(fig) & (nargin == 1)
...
elseif strcmp(cmd,'classify') & ~isempty(fig) & (nargin == 1)
