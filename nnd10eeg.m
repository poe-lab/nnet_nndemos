...
%
%  An adaptive filter removes noise from an EEG signal.  Adjusting
%  the filter's learning rate and number of tap delays results in
...
elseif strcmp(cmd,'init') & isempty(fig)
  ...
  H = nndArray({fig_axis desc_text signal_axis ...
...
elseif strcmp(cmd,'lr') & ~isempty(fig)
...
elseif strcmp(cmd,'delays') & ~isempty(fig)
...
elseif strcmp(cmd,'show_s') & ~isempty(fig)
...
elseif strcmp(cmd,'show_e') & ~isempty(fig)
